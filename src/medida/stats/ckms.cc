...
CKMSFactoryFunc createCKMS = []() { 
    return std::make_shared<CKMSImpl>(); 
};

// The default quantiles request the error be less than 0.1% (=0.001) for P99 and P50.
std::vector<CKMSImpl::Quantile> const kDefaultQuantiles = {{0.99, 0.001}, {0.5, 0.001}};

CKMS::CKMS() {
}

CKMSImpl::Quantile::Quantile(double quantile, double error)
    : quantile(quantile),
      error(error),
      u(2.0 * error / (1.0 - quantile)),
      v(2.0 * error / quantile) {}

CKMSImpl::Item::Item(double value, int lower_delta, int delta)
    : value(value), g(lower_delta), delta(delta) {}

double CKMSImpl::allowableError(int rank) {
...
CKMSImpl::CKMSImpl() : CKMSImpl(kDefaultQuantiles) {
}

CKMSImpl::CKMSImpl(const std::vector<Quantile>& quantiles)
    : quantiles_(quantiles), count_(0), buffer_{}, buffer_count_(0), 
      size_when_last_sorted_(0), max_(0) {}

std::size_t CKMSImpl::count() const {
    return count_ + buffer_count_;
}

double CKMSImpl::max() const {
    return max_;
}

void CKMSImpl::insert(double value) {
    ZoneScoped;
    if (count() == 0) {
        max_ = value;
    } else {
